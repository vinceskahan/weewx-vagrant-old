WEEWX_DOWNLOAD_METHOD="rhrpm"      # official version, Redhat rpm
...
case "x${WEEWX_DOWNLOAD_METHOD}" in
   ...
   "xrhrpm")
        ...
        #--- import rpm key for weewx ---
        echo "...defining weewx repo..."
        sudo rpm --import http://weewx.com/keys.html

        #--- install weewx in simulator mode with no prompts ---
        echo "...downloading weewx..."
        wget http://weewx.com/downloads/weewx-${WEEWX_VERSION}-${WEEWX_RELEASE}.rhel.noarch.rpm

        echo "...installing  weewx..."
        yum install -y ./weewx-${WEEWX_VERSION}-${WEEWX_RELEASE}.rhel.noarch.rpm
        ...
        # set the location to something indicating this os
        HOSTNAME=`hostname`
        sed -i -e s:Hood\ River,\ Oregon:${HOSTNAME}: /etc/weewx/weewx.conf
